...
public class ExtendedPlayerInventory extends PlayerInventory {
	 ...
	 */
	public final List<NonNullList<ItemStack>> allInventories = ImmutableList.of(this.items, this.armor, this.offhand,
			this.fishing_bag, this.quiver, this.sacks, this.potions, this.accessories);
	...
	public ItemStack removeItem(int index, int count) {
		...
		return targetInventory != null && !targetInventory.get(index).isEmpty()
				? ItemStackHelper.removeItem(targetInventory, index, count)
				: ItemStack.EMPTY;
	}
	...
	public int getContainerSize() {
		return super.getContainerSize() + this.fishing_bag.size() + this.quiver.size() + this.sacks.size()
	...
	public void load(ListNBT list) {
		...
		for (int i = 0; i < list.size(); ++i) {
			...
			final ItemStack stack = ItemStack.of(compound);
			if (!stack.isEmpty())
				for (final NonNullList<ItemStack> inv : this.specialInv)
		...
		for (final NonNullList<ItemStack> inventory : this.allInventories)
			for (int i = 0; i < inventory.size(); ++i)
				if (!inventory.get(i).isEmpty())
					inventory.get(i).inventoryTick(this.player.level, this.player, i, this.selected == i);
	...
	public void updateStartIndexes() {
		this.fishing_bag_start = super.getContainerSize();
