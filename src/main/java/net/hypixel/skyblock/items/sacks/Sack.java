...
public abstract class Sack extends Item {
	...
	protected Sack(@Nonnull Properties properties, SackSize type) {
		super(properties);
		this.type = Objects.requireNonNull(type, "SackSize cannot be null");
	...
	public void empty(@Nonnull PlayerEntity player) {
		...
		for (int i = 0; i < list.size(); i++)
			player.addItem(new ItemStack(list.get(i), this.held[i]));
	}
