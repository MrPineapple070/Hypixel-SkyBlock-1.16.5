...
public class ModExperienceBottleItem extends ExperienceBottleItem {
	...
	public ActionResult<ItemStack> use(World worldIn, PlayerEntity playerIn, Hand handIn) {
		...
		worldIn.playSound((PlayerEntity) null, playerIn.getX(), playerIn.getY(), playerIn.getZ(),
				SoundEvents.EXPERIENCE_BOTTLE_THROW, SoundCategory.NEUTRAL, .5f,
				0.4F / (random.nextFloat() * .4f + .8f));
		if (!worldIn.isClientSide) {
			...
			exp_entity.shootFromRotation(playerIn, playerIn.xRot, playerIn.yRot, -20f, .7f, 1f);
			worldIn.addFreshEntity(exp_entity);
		}
