		...
		for (final BlockPos[][] y_level : this.nearby)
			for (final BlockPos[] x_level : y_level)
				for (final BlockPos pos : x_level) {
					...
					case Potatoes:
					case Beetroot:
						((CropsBlock) block).growCrops(worldIn, pos, state);
						continue;
					case Melon:
					case Pumpkin:
						((StemBlock) block).performBonemeal((ServerWorld) worldIn, Item.random, pos, state);
						continue;
					case Cactus:
					...
					case Sugar_Cane:
						...
						continue;
					case Cocoa:
						((CocoaBlock) block).performBonemeal((ServerWorld) worldIn, Item.random, pos, state);
						continue;
					case Bamboo:
						((BambooBlock) block).performBonemeal((ServerWorld) worldIn, Item.random, pos, state);
						continue;
					case Bamboo_Sapling:
						((BambooSaplingBlock) block).performBonemeal((ServerWorld) worldIn, Item.random, pos, state);
						continue;
					default:
						continue;
					}
